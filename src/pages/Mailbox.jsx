...
const Mailbox = () => {
  ...
  const deleteLetter = async (id) => {
    ...
    try {
      ...
      else setFeedback({ message: LOADING_COPY.MB_SEND_FAIL, error: true });
    } finally {
      setDeletingId(null);
    }
  };
      ...
      <div className="grid gap-5">
        ...
        {!lettersLoading &&
          filteredLetters.map((letter) => (
            <div
              key={letter.id}
              className="bg-white p-6 rounded-[30px] shadow-sm border border-[color-mix(in_srgb,var(--om-lavender)_30%,transparent)] relative group hover:shadow-md transition-all"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-2 text-gray-300 hover:text-red-400 transition-all disabled:opacity-50"
                  title={deletingId === letter.id ? LOADING_COPY.MB_DELETING : 'Xóa'}
                >
                  <Trash2 size={16} />
                </button>
              </div>
